...
public class ArchNavigationDemoController extends BaseController {
    ...
    private int index;
    private DisplayUpMode displayUpMode = DisplayUpMode.SHOW_FOR_CHILDREN_ONLY;

    static Bundle fromBundle(int index, DisplayUpMode displayUpMode) {
    ...
    public ArchNavigationDemoController(int index, DisplayUpMode displayUpMode) {
    ...
    public ArchNavigationDemoController(Bundle args) {
        super(args);
    ...
    @OnClick(R.id.btn_next) void onNextClicked() {
      Bundle args = ArchNavigationDemoController.fromBundle(index + 1, displayUpMode.getDisplayUpModeForChild());
      Navigation.findNavController(getView()).navigate(R.id.firstController, args);
    }

    @OnClick(R.id.btn_up) void onUpClicked() {
      Navigation.findNavController(getView()).navigate(R.id.activity);
    }

    @OnClick(R.id.btn_pop_to_root) void onPopToRootClicked() {
      Navigation.findNavController(getView()).popBackStack(R.id.firstController, true);
    }
}
